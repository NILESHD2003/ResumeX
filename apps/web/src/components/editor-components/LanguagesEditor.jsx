...
const LanguagesEditor = ({resumeMetadata, setResumeMetadata}) => {
  const [selectedLayout, setSelectedLayout] = useState(resumeMetadata?.languages?.layout || 'grid');
  const [selectedSubinfoStyle, setSelectedSubinfoStyle] = useState(resumeMetadata?.languages?.subInfoStyle || 'none');
  const [selectedLayoutStyle, setSelectedLayoutStyle] = useState(resumeMetadata?.languages?.layoutInfo || 'two-column');
  ...
  const showLevelStyles = selectedLayout === 'level';
  ...
  return (
    <div className="w-full p-6 bg-white border border-gray-200 rounded-lg shadow-sm">
      <h2 className="text-lg font-semibold text-gray-900 mb-6">Languages</h2>
